...
const generateTextSchema = z.object({
    ...
    maxTokens: z.preprocess(
        ...
        z.number().int().min(64).max(1024).optional()
    ),
...
router.post('/generate-text', requireRole(['admin']), validate({ body: generateTextSchema }), async (req, res) => {
    try {
        const userId = req.session?.userId;
        if (!userId) {
            return sendError(req, res, 401, 'Not authenticated');
        ...
        const selectedModel = resolveModel(user.ai_model, aiService.model);
        const userMaxTokens = Number.isFinite(user.ai_max_tokens)
            ? user.ai_max_tokens
            : parseInt(String(user.ai_max_tokens || ''), 10);
        const maxTokensParsed = Number.isFinite(req.validatedBody.maxTokens)
            ? req.validatedBody.maxTokens
            : (Number.isFinite(userMaxTokens) ? Math.min(1024, userMaxTokens) : 256);
        const maxTokens = Math.max(64, Math.min(1024, maxTokensParsed));
        const temperature = (typeof req.validatedBody.temperature === 'number')
